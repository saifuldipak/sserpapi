...
def create_user(user_schema: schemas.User, db: Session = Depends(get_db)):
    user_exists = db_query.get_user_by_name(db, user_name=user_schema.user_name)
    if user_exists:
        raise HTTPException(status_code=400, detail="User exists")
    return db_query.add_user(db=db, user_schema=user_schema)
